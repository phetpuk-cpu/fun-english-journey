...
> ⚠️ **อัปเดต 11 ก.ค. 2026: เลขชั้นในเอกสารนี้ล้าสมัยแล้ว** — เลื่อนป้ายชั้นทั้งระบบ: ป.3→ป.2, ป.4→ป.3, ป.5→ป.4 (เนื้อหา/ธีมเดิมทุกอย่าง) **ชื่อไฟล์ (lessonId) ที่ระบุในตารางด้านล่างนี้ยังใช้ prefix เดิม (p3u.../p4u...) — ให้แปลงตามกฎ p3→p2, p4→p3 ก่อนตั้งชื่อไฟล์จริงเสมอ** เช่น `p3u1l1-scene.png` ในตาราง = ต้องบันทึกเป็น `p2u1l1-scene.png` จริงในโฟลเดอร์ assets/ ดูรายละเอียดที่ `curriculum-plan-p1-2-p5-6.md`
