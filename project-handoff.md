...
อัปเดต: 17 ก.ค. 2026 (เวอร์ชัน 9 — **หลักสูตร ป.1-6 ครบ 204 บทเรียน (เพิ่มหน่วย 9-10 "มุมนิทาน" อ่านจับใจความ 12 บท) + เสียงครบ 100% ทุกชั้น (192 บทเดิม) + DEPLOY ขึ้นเว็บจริงแล้ว!** 🎉 https://phetpuk-cpu.github.io/fun-english-journey/ — เหลือแค่งานภาพที่ Codex/Antigravity ทยอยผลิต ซึ่งเว็บอัปเดตเองอัตโนมัติเมื่อ push)
...
**สถานะรวมตอนนี้:** ป.1-6 มีเนื้อหาเต็มครบ **204 บทเรียนทั้งหมด** (192 บทเดิม + 12 บท "มุมนิทาน" หน่วย 9-10 ใหม่) เล่นได้จริงในแอปแล้วทั้งหมด มี 6 แท็บในหน้าแผนที่ (🌻 ป.1 / 🏙️ ป.2 / 🎪 ป.3 / 🌍 ป.4 / 🏆 ป.5 / 🚀 ป.6) — **หลักสูตรเนื้อหาเขียนจบสมบูรณ์แล้ว** สิ่งที่เหลือทั้งหมดตอนนี้คืองานภาพ/เสียง (Codex/Antigravity) และงาน polish/deploy (บทมุมนิทาน 12 บทใหม่ยังไม่มีเสียง/ภาพประกอบ ใช้ TTS+emoji เหมือนบทอื่นที่รอภาพอยู่)
...
- [x] **แบบทดสอบจำลอง Cambridge YLE (mock exam)** — เสร็จแล้ว 12 ก.ค. 2026 หน้าใหม่ `#scr-exam`
  สุ่ม 20 ข้อจาก quiz ที่มีอยู่แล้วทั้งชั้น (ไม่ต้องเขียนเนื้อหาใหม่) ให้คะแนนแบบ shield 1-5 อัน พร้อมข้อความกันสับสนว่า
  "ไม่ใช่ผลสอบจริง" ทดสอบ end-to-end แล้ว (20/20 → 5 shields, XP อัปเดตถูกต้อง) — **17 ก.ค. 2026 ย้ายจุดเข้าถึงจากปุ่ม 📝 topbar
  ไปเป็นการ์ดท้ายลิสต์บทเรียนของแต่ละชั้น** (หลังหน่วยสุดท้ายรวมหน่วย 9-10 ใหม่) ให้เป็น "บทสรุปท้ายชั้น" ที่เจอเป็นธรรมชาติแทนปุ่มลอยกดได้ทุกเมื่อ
- [ ] เพลงประจำหน่วย — **คอนเซปต์ครบ 50 เพลงร่างเสร็จแล้ว 19 ก.ค. 2026** (`unit-songs-brief-gemini.md`: 48 เพลงหลัก + 2 เพลงอนุบาล พร้อม prompt แม่แบบ/สเปกกลาง/กฎลิขสิทธิ์/ชื่อไฟล์ส่งมอบ) → รอผู้ใช้ generate ด้วย Gemini ทีละชั้น แล้ว Claude ตรวจ+เพิ่มปุ่ม 🎵 ที่หัวหน่วยตอนไฟล์ชุดแรกมาถึง
- [ ] กิจกรรม Roleplay Chat สำหรับ ป.4 ขึ้นไป
- [x] **โหมดอนุบาล 🐣 (Phonics/ABC/CVC) — ครบ 8 หน่วยแล้ว 20 ก.ค. 2026** engine phonics 5 step type + แท็บ 🐣 + เนื้อหาครบทุกหน่วย (หน่วย 1-4 ตัวอักษร a-z, หน่วย 5-8 ตระกูลคำ CVC 4 ตระกูล) รวม 28 บท + asset ครบ (เสียง 108 + ภาพ 112 + เพลง 2) ทดสอบ end-to-end บน production ผ่านทุกหน่วย (เช็คการข้ามหน่วย, บททบทวน, asset reference ครบทุกตัวอักษร/คำ) — **เหลือ: รอเด็กอนุบาลจริงทดสอบใช้งานจริงแล้วเก็บฟีดแบ็กมาปรับ** (ดีไซน์เดิมด้านล่างเก็บไว้อ้างอิง)
  - **หลักสูตร**: แท็บใหม่ grade key `k` · หน่วย 1-4 = ตัวอักษร a-z (ชื่อ+เสียง phoneme+คำตัวอย่าง) · หน่วย 5-8 = ตระกูลคำ CVC (-at/-an/-ap/-ig/-og/-ug/-et/-en/-ed รวม 30 คำ)
  - **หลักออกแบบ**: audio-first ทั้งหมด (เด็กอนุบาลอ่านหนังสือไม่ออก — engine เดิมใช้ตรงๆ ไม่ได้) ต้องมี step type ใหม่ ~4-5 แบบ: รู้จักตัวอักษร, ฟังแล้วแตะ, จับคู่เสียง-ภาพ, blending (c-a-t→cat โดย engine ประกอบเสียงจากไฟล์ phoneme), พูดตาม (เกณฑ์ต่ำกว่าปกติ)
  - **บรีฟที่ออกแล้ว**: `asset-production-kit-batch8-kindergarten-images.md` (Codex, ~56 ภาพ, โฟลเดอร์ใหม่ `assets/img/phonics/` + ต้องเพิ่ม category ใน optimize-images.sh) และ `asset-production-kit-batch8-kindergarten-audio.md` (Antigravity, 108 ไฟล์เสียง) — รายการคำ/ชื่อไฟล์ล็อกตรงกันสองไฟล์แล้ว
  - **⚠️ Blocker จริง**: เสียง phoneme 26 ไฟล์จาก Antigravity — TTS เบราว์เซอร์ fallback ไม่ได้ (ออกเสียง /b/ เดี่ยวๆ ไม่เป็น) โหมดนี้เปิดใช้ไม่ได้จนกว่าเสียงครบ
  - **แผนสร้าง**: ทำ pilot 2 หน่วยก่อน (หน่วย 1 ตัวอักษรกลุ่มแรก + หน่วย 5 ตระกูล -at) ให้เด็กอนุบาลจริงทดสอบ แล้วค่อยขยายครบ 8 หน่วย — งาน engine ฝั่ง Claude ~1-1.5 วัน + เนื้อหา ~1 วัน
- [x] Phase 4b: แดชบอร์ดผู้ปกครองขั้นสูง — เสร็จแล้ว 13 ก.ค. 2026 (`parents-dashboard.js`) เพิ่ม progress bar ต่อโปรไฟล์ ("เรียนไปแล้ว X/34 บท"), จุดที่ควรฝึกฝนเพิ่ม (บทที่ได้ 1-2 ดาว), สรุปกิจกรรม 7 วันล่าสุด (จำนวนครั้ง+คะแนนเฉลี่ยพูด+เทียบเทรนด์), ปุ่มพิมพ์สรุป (`window.print()` แทนอีเมล/LINE อัตโนมัติที่ต้องมี backend) — deploy แล้ว
- [x] **หน่วย 9-10 "มุมนิทาน" (reading-only units) ทุกชั้น** — เสร็จแล้ว 17 ก.ค. 2026 เพิ่ม 12 บทเรียนใหม่ (6 ชั้น × 2 บท)
  เป็นบทอ่านจับใจความล้วน (ไม่มีศัพท์/พูด/quiz) ใช้ step type `"reading"` เดิมที่มีอยู่แล้ว ผูกเนื้อเรื่องกับตัวละคร Ducky/Mali/Teacher Owl/Grumpy Cat
  และไวยากรณ์ที่แต่ละชั้นเรียนไปแล้วในหน่วย 1-8 — เพิ่ม field มาร์คเกอร์ `lessonType:"reading"` ใน schema, แก้ engine.js/validate-lessons.js/parents-dashboard.js
  รองรับ (ดูรายละเอียดใน git log) — รวมบทเรียนทั้งระบบเป็น **204 บท** (จากเดิม 192), 60 ไฟล์ข้อมูล (จากเดิม 48)
